...
GethApiDouble.prototype.getContractStorage = function(contractAddr, key, callback) {
  const vmAddr = Buffer.from(contractAddr, 'hex');
  const vmKey = Buffer.from(key, 'hex');
  var self = this;
  self.state.blockchain.vm.stateManager.getContractStorage(vmAddr, vmKey, function(err, ret){
    console.log(ret.toString('hex'));
    callback(err, ret);
  });
};

GethApiDouble.prototype.putContractStorage = function(contractAddr, key, value, callback) {
  // putContractStorage(address: Buffer, key: Buffer, value: Buffer, cb: any):
  const vmAddr = Buffer.from(contractAddr, 'hex');
  const vmKey = Buffer.from(key, 'hex');
  const vmValue = Buffer.from(value, 'hex');
  var self = this;

  self.state.blockchain.vm.stateManager.putContractStorage(vmAddr, vmKey, vmValue, function(err, ret){
    callback(err, ret);
  });
