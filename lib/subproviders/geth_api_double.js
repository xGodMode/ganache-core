...
/**
 * Logs `testInput` in debug mode and passes it to `callback`
 * @param {any} testInput
 * @param {function} callback
 */
GethApiDouble.prototype.godmode_debug = function(testInput, callback) {
  debug(testInput);
...
GethApiDouble.prototype.handleRequest = function(payload, next, end) {
  debug(`GethApiDouble.prototype.handleRequest: payload.method: ${payload.method}`);
...
// TODO: Can we use getCode here?

/**
 * Passes contract bytecode to `callback` or passes an error if one occurs
 * @param {string} contractAddr Ethereum address as Buffer
 * @param {function} callback
 */
GethApiDouble.prototype.godmode_getContractCode = function(contractAddr, callback){
  const vmAddr = Buffer.from(contractAddr, 'hex');
  this.state.blockchain.vm.stateManager.getContractCode(vmAddr, function(err, ret){
    callback(err, ret);
  });
};

/**
 * Replaces code found at `contractAddr` with `value`
 * @param {string} contractAddr Ethereum address as Buffer
 * @param {string} value Bytecode as Buffer
 * @param {function} callback
 */
GethApiDouble.prototype.godmode_putContractCode = function(contractAddr, value, callback){
  const vmAddr = Buffer.from(contractAddr, 'hex');
  const vmValue = Buffer.from(value, 'hex');
  this.state.blockchain.vm.stateManager.putContractCode(vmAddr, vmValue, function(err, ret){
    callback(err, ret);
  });
};
...
/**
 *
 * @param {string} contractAddr Ethereum address
 * @param {string} key Storage location
 * @param {function} callback
 */
GethApiDouble.prototype.godmode_getContractStorage = function(contractAddr, key, callback) {
  const vmAddr = Buffer.from(contractAddr, 'hex');
  const vmKey = Buffer.from(key, 'hex');
  this.state.blockchain.vm.stateManager.getContractStorage(vmAddr, vmKey, function(err, ret){
    callback(err, ret);
  });
};
 ...
 * @param {function} callback
 */
GethApiDouble.prototype.godmode_putContractStorage = function(contractAddr, key, value, callback) {
  const vmAddr = Buffer.from(contractAddr, 'hex');
  const vmKey = Buffer.from(key, 'hex');
  const vmValue = Buffer.from(value, 'hex');
  this.state.blockchain.vm.stateManager.putContractStorage(vmAddr, vmKey, vmValue, function(err, ret){
    callback(err, ret);
  });
};
...
/**
 * Locks any unlocked account
 * @param {string} account Ethereum address
 * @param {function} callback
 */
GethApiDouble.prototype.godmode_lockAccount = function(account, callback) {
  var account = this.state.unlocked_accounts[address.toLowerCase()];
  if (account !== true) {
    var error = "Account not found";
    return callback(error);
  }
 ...
 * @param {function} callback
 */
GethApiDouble.prototype.godmode_unlockAccount = function(account, callback) {
  this.state.unlocked_accounts[account.toLowerCase()] = true;
  callback(null, true);
};
